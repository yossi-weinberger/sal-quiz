  ...
  return (
    <div
      className={`
        ...
        ${hasBranch ? "bg-green-50 border-green-200" : "bg-amber-50 border-amber-200"}
      `}
    >
      <h3 className="font-semibold text-sm mb-2">
        {resultsContent.city.title}
      </h3>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {branches.map((branch) => (
              <li
                key={branch.id}
                className="bg-white/60 rounded-lg border border-green-200 px-3 py-2"
              >
                ...
                {branch.format_type && (
                  <p className="text-xs text-green-700 mt-0.5">
                    {resultsContent.city.branchFormat.replace(
                      "{{format}}",
                      branch.format_type
                    )}
                  </p>
